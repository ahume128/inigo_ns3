...
int main (int argc, char *argv[])
{
  ...
  std::string delay = "0.05ms";
  std::string access_bandwidth = "500Mbps";
  std::string access_delay = "0.05ms";
  bool tracing = false;
  std::string prefix_file_name = "TcpVariantsComparison";
  double data_mbytes = 0;
  uint32_t mtu_bytes = 1500 + 60;
  ...
  prefix_file_name = prefix_file_name + "_" + transport_prot;
  ...
  //LogComponentEnable("TcpCongestionOps", LogLevel(LOG_LEVEL_ALL | LOG_PREFIX_NODE | LOG_PREFIX_FUNC | LOG_PREFIX_TIME));
  ...
  // 4 MB of TCP buffer
  Config::SetDefault ("ns3::TcpSocket::RcvBufSize", UintegerValue (1 << 15));
  Config::SetDefault ("ns3::TcpSocket::SndBufSize", UintegerValue (1 << 15));
  ...
  else if (transport_prot.compare ("TcpInigo") == 0)
    {
      Config::SetDefault ("ns3::TcpL4Protocol::SocketType", TypeIdValue (TcpInigo::GetTypeId ()));
    }
  ...
  for (uint16_t i = 0; i < sources.GetN (); i++)
    {
      ...
      if (transport_prot.compare ("TcpNewReno") == 0
          ...
          || transport_prot.compare ("TcpInigo") == 0
  ...
  if (tracing)
    {
      ...
      Simulator::Schedule (Seconds (0.1), &TraceQueue, prefix_file_name + "-q.data");
    }
